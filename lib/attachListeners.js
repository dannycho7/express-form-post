...
module.exports = function(busboy, req) {
	...
	busboy.on("finish", () => {
		...
		.then(() => {
			if(Object.keys(req.efp.streams).length == 0) return this.finished();
			// check for min size
			for(var key in req.efp._data) {
				if(this.opts.minfileSize > req.efp._data[key]) {
					return this.handleError(new Error("Uploaded file was smaller than minfileSize."
						+ " the file was of size " + req.efp._data[key]));
				}
			}
			// save files after all validation
			for(var stream in req.efp.streams) {
				req.efp.streams[stream].end();
			}
		})
		.catch((err) => {
